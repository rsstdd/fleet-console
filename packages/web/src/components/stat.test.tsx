...
describe("Stat", () => {
  it("renders value then label in visual and DOM order without redundant ARIA", () => {
    ...
    expect(stat()).not.toHaveAttribute("aria-label");
    expect(stat().children[0]).toHaveClass("stat__value", "mono");
    expect(stat().children[0]).toHaveTextContent("44");
    expect(stat().children[1]).toHaveClass("stat__label");
    expect(stat().children[1]).toHaveTextContent("Live");
  });

  it("renders a non-empty hint after the label", () => {
    render(<Stat label="Live" value="44" hint="of 50" />);

    expect(stat().children[2]).toHaveClass("stat__hint");
    expect(stat().children[2]).toHaveTextContent("of 50");
  });

  it("omits an empty hint", () => {
    render(<Stat label="Unknown" value={0} hint="" />);

    expect(stat().querySelector(".stat__hint")).toBeNull();
  });
