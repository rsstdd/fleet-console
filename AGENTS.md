...
- Source comments are exceptional: add one only for an important invariant, external-system lifecycle, workaround, non-obvious protocol/domain constraint, or caller-visible behavior that names, types, tests, and owning documentation cannot express (ADR 39). Export, layer crossing, composition, hooks, and constants do not create a comment requirement. Any doc comment that exists must not restate its signature (ADR 28).
- Express cross-file and cross-package coupling through shared types, APIs, tests, or owning documentation. If a load-bearing relationship remains easy to violate, document it once at the least-obvious side rather than mirroring comments (ADR 39, Principle 14).
...
- Every mechanical rule is registered under `mechanicalRules` in `docs/decisions.json`. That registration is the only binding path → ADR record: the number is not copied into a comment beside the mechanism, and CI does not look for one. Code states the constraint it enforces, never the ADR's rationale.
- TypeScript packages inherit `tsconfig.base.json` with `strict: true`, exact optional-property semantics, and the additional checked-index, return, override, unused, and switch rules. Typed ESLint must reject explicit `any` and members that can be readonly; use a precise type or `unknown` plus boundary validation (ADR 38). Run `pnpm check:type-safety` after changing TypeScript or ESLint configuration.
