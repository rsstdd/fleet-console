...
- **20 August 2026 — what the sequential number is not.** It is a round trip over loopback including the client's own `fetch`, so it bounds server-side per-request work from above rather than isolating it. The later concurrent run above supplies the separate capacity evidence: 5,971 requests/s with no late sweep ticks at concurrency 128.
- **20 August 2026 — cold start is implemented and tested.** `GET /api/fleet` serves every manifest robot with the same process session and flush-sequence source as `/ws`. The client opens the socket, buffers, fetches, then reconciles; contract, unit, process, and ADR 32 browser tests cover the ordering and restart path.
...
- Artifact `packages/server` — implemented ingest, dispatch, snapshot reads, and fan-out live here.
