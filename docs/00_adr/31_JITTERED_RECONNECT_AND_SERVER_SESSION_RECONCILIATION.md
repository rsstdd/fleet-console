...
- **When does a completed join stop counting as success?** The backoff reset is unconditional, so a server that accepts, serves the snapshot and drops the stream in a loop is indistinguishable from a healthy one to the schedule. Measured 26 August 2026 against the transport with injected time: six drop cycles opened **seven sockets and scheduled zero timers**, each cycle paying a full `GET /api/fleet`. § Argument justifies counting the join rather than the open precisely to stop a half-working deployment resetting its backoff every cycle; this reaches the same end by the route the rule permits. Candidate answers: a minimum session duration before the reset applies; counting consecutive joins that delivered no frame; or treating the loop as a deployment failure the health endpoint should surface. Each introduces a tuning value, which is why none is implemented.
