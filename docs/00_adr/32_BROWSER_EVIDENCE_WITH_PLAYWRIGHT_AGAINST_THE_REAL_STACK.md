...
- **26 August 2026 — the integrity wait was still host-sensitive, and the timing numbers on this host are no longer comparable.** The 23 August fix made the _counter_ synchronous; the _wait_ remained `expect.poll` over `page.evaluate`, and every one of those round trips queues behind the same saturated main thread the render loop is using. On a contended host the run failed with `received` provably correct — 120 of 120, verified by evaluating it directly — and a screenshot showing the final frame rendered: the 23 August symptom exactly, one layer up, and again indistinguishable from a regression. Unmodified runs alternated pass and fail; adding any single `page.evaluate` before the poll made them pass. Replaced with `page.waitForFunction` on a 100 ms timer — in the page, no round trip per attempt, and not the default animation-frame polling, which is starved precisely when this matters — with the exact-count assertion kept as one evaluate afterwards. Three consecutive runs then passed.

  Separately, and not a console change: this host now reports **animation-frame interval p50 166.7 ms** against the 16.7 ms of 20 August, on the same WSL2, Chromium 151, 1280×720, 8 logical CPUs. Delta-to-next-paint is measured receipt-to-next-animation-frame, so it is bounded below by that cadence, and the p50 140–149 ms / p95 171–199 ms reported across three runs describes the harness environment rather than the console. **These runs are integrity evidence, not timing evidence**, and must not be compared with the 20 August numbers or used to reopen ADR 18 or ADR 24. A comparable timing measurement needs a host whose animation-frame cadence is healthy — CI, or a quiet machine.
