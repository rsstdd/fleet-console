...
- **The 50 Hz coupling runs both ways.** Raising the simulator's validated ceiling without raising `MAX_SOURCE_RATE_HZ` silently shortens the covered window below the contract's 60 seconds; the least-obvious side carries the warning and this ADR owns the rationale (ADR 39).
