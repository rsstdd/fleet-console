...
- **What does the server do when `FLEET_ALLOWED_ORIGINS` is set and the request has no `Origin` header?** Same-origin and non-browser callers send none. The intended answer is "allow, because it is not a cross-origin request", but that belongs with the middleware, not here.
