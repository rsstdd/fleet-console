...
- **20 August 2026 — the composition root exists, so `loadRuntimeEndpoints()` finally has a caller.** `packages/server/src/main.ts` binds what it returns and terminates on `ConfigValidationError` rather than continuing on defaults: an out-of-range `FLEET_SERVER_PORT` exits 1 naming the key and the accepted range on stderr, and never binds. The § Implications line that this ADR "removes the configuration blocker, not the missing process" is now discharged — `pnpm dev` starts a server, and the simulator's posts reach a process rather than a closed port, though they still 404 because no route is mounted. `docs/ARCHITECTURE_AUDIT.md` § 2 is closed on the process and still open on the route.
