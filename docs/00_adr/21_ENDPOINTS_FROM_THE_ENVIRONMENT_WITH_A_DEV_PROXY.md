...
- **`packages/server` starts from the validated endpoint configuration.** `main.ts` loads it before any background work, and startup stops on `ConfigValidationError` rather than continuing with guessed values.
- **`FLEET_ALLOWED_ORIGINS` is enforced.** The mounted origin policy echoes an allowed origin, varies the response by `Origin`, and handles preflight without granting an unlisted origin.
...
- **The one-command path is live.** `pnpm dev` starts the server, simulator, and console on their matching defaults; the historical architecture audit remains a record of the earlier missing process.
