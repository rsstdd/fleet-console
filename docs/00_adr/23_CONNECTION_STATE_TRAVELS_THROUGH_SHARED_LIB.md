...
- **The app-owned transport now supplies real connection state.** Labels render only after the socket and snapshot join succeeds; missing composition still fails closed to an empty Reporting status column and a disconnected banner.
...
- **20 August 2026 — the fleet-summary question closed the way the resolution above records.** Six fleetPage tests landed first and five failed red (connected heading unqualified, reconnecting and disconnected qualified, counts unchanged while down, filtering leaves fleet-wide counts and qualification untouched, one h1 then the summary h2); the implementation is a `section` labelled by the heading, reading the same `streamConnected` the row suppression already used — no new state, no Stat API change. The Playwright outage scenario (ADR 32) now also asserts the browser truth: after `stopServer()` the region named "Fleet reporting status · last known" holds the heading and all four stat labels. One test-authoring trap surfaced: a bare `/last known/` text query matches every suppressed row's "Idle (last known)" chip, so the absence assertion is scoped to headings.
- **21 August 2026 — wording amendment, position unchanged.** The operator-facing copy was renamed from "Fleet freshness" / "Freshness" to "Fleet reporting status" / "Reporting status" (plan `FLEET_REPORTING_STATUS_COPY`); the quoted heading and column strings in this ADR were amended to match. At the time the entries above were written the copy read "Fleet freshness". The decision itself — qualify, don't suppress; one shared heading; no per-metric tag, aria-live region, or client timestamp — is untouched, and the domain term "freshness" (ADR 3) remains the internal vocabulary.
