...
- **The fleet summary still counts freshness states while disconnected.** See Open questions — this is a real inconsistency that this ADR deliberately did not resolve.
...
- **Should the fleet summary's freshness counts be suppressed too?** While disconnected the table shows no per-robot labels but the summary still reads "Live 1 · Stale 1 · …". Arguably that is the same currency claim at fleet scope. Fleet spec § 8 says only that per-robot labels are suppressed, so resolving this is a spec change rather than an implementation choice, and it was left rather than taken unilaterally. It is recorded in `features/fleet/TODO.md` as **A7**.
