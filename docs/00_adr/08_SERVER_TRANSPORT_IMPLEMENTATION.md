...
- **20 August 2026 — one port serves both, and the shutdown order is asserted rather than described.** `startListener` binds Hono through `@hono/node-server` and attaches `ws` to the returned server's `upgrade` event, exactly as this ADR's § Argument anticipated. Its `close()` closes stream clients, then the socket server, then the HTTP server; the test does not inspect that order but **rebinds the same port afterwards**, which is the only assertion that distinguishes a shutdown that finished from one that resolved early.
- **20 August 2026 — the upgrade is scoped to `/ws` with `noServer: true`, and is not origin-checked.** Handing `ws` the whole server would upgrade any path into a stream the fan-out never writes to, so a request elsewhere is destroyed instead. The gap this exposes is that CORS governs `/api` and nothing governs the handshake: `ws` checks no origin of its own, so `/ws` is reachable from any origin. Whether the ADR 21 allow-list should govern the stream is **undecided** and flagged in `packages/server/TODO.md` § Section 2; it belongs with the fan-out work, not with a line in the upgrade handler.
- **20 August 2026 — `ServerType` widened the `upgrade` handler's arguments to `any`.** It is a union of Node's HTTP and HTTP/2 servers, whose `on` overload does not recognize the event, and this package's `no-unsafe-argument` rules rejected the result. The parameters are annotated from `node:http` and `node:stream` instead. Worth recording because the adapter layer this ADR called "the honest price of the choice" charged its first instalment here, and it charged it in types rather than in behaviour.
