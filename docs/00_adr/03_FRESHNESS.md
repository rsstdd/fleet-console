...
- **20 August 2026 — the saturation failure this ADR names was looked for and not found.** § Implications warns that under ingest saturation the sweep stops firing and the console freezes robots at their last computed state instead of degrading them. Driving concurrent ingest to ~6,000 req/s at 500 robots produced **zero late sweep ticks** across every level, and the interval was still running afterwards. Worth two caveats. First, "not found" is a statement about this machine and this offered load, not a proof that the interval cannot be starved. Second, the late-tick detector was therefore **unexercised by that run** — it is watched by a separate manual-clock test in `runServer.test.ts` that forces a late tick and asserts both the counter and the `freshness.tick_late` warning, which is what keeps it from being a guard nobody has seen fire (ADR 7).
- **20 August 2026 — the guarantee is observable by a client for the first time.** Against a running server: a console opened `/ws`, fetched `GET /api/fleet` at flush sequence 0, posted one vendor A payload, and then received two frames — sequence 1 carrying `R-001` as `live`, and sequence 2 carrying the same robot as `stale` with `reportedAt` unchanged. The second frame came from the sweep alone, with no telemetry behind it, which is the whole of **F4**: a robot that only aged is a change worth sending. The snapshot then reported sequence 2 from the same counter the frames carried.
- **20 August 2026 — the sweep is composed, observable, and queryable.** `startServer` builds the store, delta set, `HealthMetrics`, and `FreshnessSweep` together, routes `onLateTick` into the counter, emits `freshness.tick_late`, and serves the counter from `GET /api/health` through ADR 25's contracts-owned response. The sweep starts after the listener binds and stops before it closes, so no interval outlives the port.
- **20 August 2026 — the freshness-only delta is proven end to end.** A robot aged past `staleThresholdMs` enters the pending set with a new `freshness` and an untouched `reportedAt`; the fan-out drains that change onto `/ws`. The running-server proof above observes the resulting live-to-stale frame, and ADR 32 adds the real-browser transition.
