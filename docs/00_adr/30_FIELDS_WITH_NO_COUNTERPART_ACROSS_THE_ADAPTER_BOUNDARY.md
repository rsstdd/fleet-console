...
- **Which identifier space keys the health response's adapter column?**
  _Current lean:_ `SupportedVendor`, because the ledger already uses it and the registry can map to `adapterId` for display. Not decided here — it is the registry's call.
  _Resolves on:_ **C8** landing in `packages/adapters` and the server's health handler consuming it.
