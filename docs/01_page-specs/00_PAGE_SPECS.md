...
| `/map` (Map)                 | `04_MAP.md`          | `web/src/features/map` (planned) |
...
| Invariant                      | Principle | Fleet     | Robot detail | Map           | Shell     |
| ------------------------------ | --------- | --------- | ------------ | ------------- | --------- |
| Freshness visible with state   | 4         | ✓ rows    | ✓ header     | ✓ marker fill | —         |
| Status labelled chips          | 6         | ✓         | ✓            | ✓ list rows   | —         |
| Connection honesty             | 5         | via shell | via shell    | via shell     | ✓ banner  |
| Async state set defined        | 5         | ✓ § 10    | ✓ § 10       | ✓ § 10        | ✓ § 8     |
| Local view state kept separate | 11        | ✓ filters | ✓ persona    | ✓ site facet  | —         |
