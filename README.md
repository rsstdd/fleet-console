...
| Layout      | pnpm workspace; web package in the community-standard React layout — `features/` (including the DEV-only component gallery), `components/`, `hooks/`, `stores/`, `context/`, `lib/`, `utils/`, `types/`, `config/` (ADR 36; folder map in [`packages/web/README.md`](packages/web/README.md)) |
...
**Agent governance:** `AGENTS.md` is the authoritative source for hard rules and the routing table (what → which package); `CLAUDE.md` is a compatibility pointer to `AGENTS.md` (Principle 14). Per-package overrides live alongside code. Comments are exceptional: they preserve only non-obvious contracts, invariants, constraints, or failure modes that code, types, tests, and owning documentation cannot express. Export location and cross-package coupling do not trigger mirrored prose (ADR 39); any doc comment that exists must remain informative (ADR 28).
